...
{
    "scripts": {
        "dev": "kizuna deprecations src/contract/index.ts && next dev",
        "build": "kizuna deprecations src/contract/index.ts && next build"
    }
}
```
...
Run `--dts` if consumers resolve your built `.d.ts`. If they read your `.ts` source directly, the JSDoc is already there.
...
When your app starts, `generateOpenApi` reads the `.kizuna/` folder from wherever the app runs. Your build's `kizuna deprecations` step creates it; it has to still be there at runtime.

On most hosts it already is: a VM, a single container, `next start`, or Railway/Render (Nixpacks) all run from the same files the build produced, so `.kizuna/` comes along.
...
It merges into `outputFileTracingIncludes`, so it composes with other config wrappers. Run `kizuna deprecations` from the same directory as `next build` so `.kizuna/` sits at the project root.
...
`.kizuna/` lands wherever you run `kizuna deprecations`. If you run it from the repo root instead of the app, set `outputFileTracingRoot` to that root. `withKizuna` derives the include path from it:
...
```bash
ts-kizuna-swift generate --contract src/contract/index.ts --output APIClient.swift --namespace-name API
```
