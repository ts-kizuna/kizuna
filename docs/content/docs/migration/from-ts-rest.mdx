...
`initServer` does not exist in ts-kizuna. Bind the contract with `new KizunaServer()`, use `server.router` to bind implementations, and `new KizunaApi()` to produce a mountable API object.
...
In ts-kizuna, `api.mount(app)` is a method on the `api` object (produced by `new KizunaApi()`) rather than a function taking separate routes and handler arguments.
