...
`initContract` does not exist in ts-kizuna. Create your surface with `new Kizuna()` once, then define groups with `k.routes`.
...
`initServer` does not exist in ts-kizuna. Create the server with `new KizunaServer()`, use `server.router` for the implementations, and `server.api` to produce a mountable API object.
