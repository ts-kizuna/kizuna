...
For request-scoped values handlers need, use [`Kizuna.requestContext`](/docs/reference/kizuna-request-context). For other middleware — logging, rate limiting, caching headers — use Hono's own `app.use`. Authentication belongs in [guards](#guards).
