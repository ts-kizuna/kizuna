...
For request-scoped values handlers need, use [request context](/docs/request-context). For other middleware, such as logging, rate limiting, and cache headers, use Next.js's own [middleware](https://nextjs.org/docs/app/building-your-application/routing/middleware), or the [`requestMiddleware`](#requestmiddleware) option to run middleware after route matching. Authentication belongs in [guards](#guards).
