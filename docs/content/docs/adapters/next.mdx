...
A guard receives the credential extracted and typed, alongside the native `request`. See the [Auth guide](/docs/authentication).
...
Middleware that sets custom properties on the request, such as a request id, needs module augmentation to extend `NextRequest` so handlers reach them without casts. Auth data does not: guards hand handlers typed context directly.
