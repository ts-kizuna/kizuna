...
`k.auth.tools` takes the same values a route's entry takes: an identity name, several, or one narrowed by an access field. A tool naming a route with `fromRoute` gets no entry, because that route's own line already says who may call it.
...
Or take a whole group, when a model should reach all of it:
...
Routes that cannot be tools, because they stream or read a form body, are simply not in the result. Spread it when one entry wants different words:
...
`k.tools` hands the builder both. `k.tools.fromRoute` and `k.tools.fromRoutes` are the same functions, for when you want one on its own.
