...
A tool that changes something, or reads someone's data, needs to know who is asking. Say so in the auth map, beside the routes:
...
```ts title="server/tools.ts"
purgeCache: ({ auth }) => dropReportsFor(auth.member.workspaceUserId),
```

`auth.member` is typed, with `role` narrowed to `'owner'`. A caller holding `admin` is refused before the handler runs, and so is a caller holding nothing.

`k.auth.tools` takes the same values a route's entry takes: an identity name, several, or one narrowed by an access field. A tool naming a route with `fromRoutes` gets no entry, because that route's own line already says who may call it.
...
export const tools = k.tools(({ fromRoutes }) => ({
    users: {
        ...
        list: fromRoutes(routes.users.listUsers),
    },
}));
```

Hand it a whole group when a model should reach all of it:
...
Routes that cannot be tools, because they stream or read a form body, are simply not in the result.

A second argument says what differs. `false` leaves a route out, and an object gives the model different words:
...
`k.tools` hands the builder a `fromRoutes`. `k.tools.fromRoutes` is the same function, for when you want it on its own.
...
```ts
fromRoutes(routes.assistant.reply);
//        ~~~~~~~~~~~~~~~~~~~~~
...
```ts title="tools.ts"
archive: fromRoutes(routes.users.archiveUser, {
    description: 'Archive a user. They keep their data and lose access immediately.',
}),
```

`title` and `annotations` take an override the same way, on a single route or on one inside a group.
...
Every tool in the tree is published, both the ones you declared and the routes you named with `fromRoutes`.
