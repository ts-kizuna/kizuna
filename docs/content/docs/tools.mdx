...
A tool that reads someone's data, or changes it, needs to know who is asking. It takes that from the route behind it.
...
export const tools = k.tools(({ fromRoute }) => ({
    workspace: {
        members: fromRoute(workspaceRoutes.listMembers),
        remove: fromRoute(workspaceRoutes.deleteWorkspace),
    },
}));
```

`remove` requires a `member` whose `role` is `owner`, because its route does. A caller holding `admin` is refused, over MCP and in process alike. Nothing about that is written twice.

The workspace itself never becomes an `input` field. The model chooses `input`, so a selector declared there is a selector the model picks. It comes from the caller's identity instead, which the route handler already receives under `auth`.

A tool you declare inline has no route, so it has no caller and no role. Keep those to computation. Anything that touches your data is a route, and `fromRoute` puts it in front of a model.
