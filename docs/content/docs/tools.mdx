...
## Take the caller, never ask for it

Give the group an identity and every handler in it receives that identity's context under `auth`, typed exactly as a route handler's is:

```ts title="tools.ts"
export const tools = k.tools('member', {
    workspace: {
        listMembers: {
...
```ts title="server/tools.ts"
listMembers: ({ input, auth }) => membersOf(auth.member.workspaceId, input.role),
```

A tool never authenticates. By the time it runs, whoever called it has been verified: the MCP transport ran the guard, or the route handler's own guard already did. That context is bound to the runner before your handler sees it.

This is why the workspace is not an `input` field. Input is what the model chooses, so a selector declared there lets the model pick whose data it reads. `auth` arrives out of band, and a tool requiring an identity refuses to run without one.

Outside a request there is no guard to run, so bind one yourself:

```ts
const asMember = tools.as({
    member: {
        workspaceId: 'w_1',
        role: 'owner',
    },
...
await asMember.workspace.listMembers.run({});
```
...
Most of what a model needs to do, your API already does. A route has an input schema, an output schema, a handler, and an identity it requires, which is everything a tool is. Restating one as a tool means two declarations to keep in step, and a stream route very often needs to look up something a route already serves.

Name the route instead:
...
`k.tools` hands the builder a `fromRoute`, so a tree that is already inside `k.tools` does not repeat it on every line. `k.tools.fromRoute` is the same function, for when you want it once.

Nothing is restated. The tool takes the route's path parameters, query and body as its arguments, answers with the route's own responses, describes itself from the route's summary, and runs the route's handler. Its `readOnlyHint` and `destructiveHint` come from what RFC 9110 says about the method, so a `GET` is marked safe and a `DELETE` destructive without you saying so.

The key is yours, not the route's. `find` reads better to a model than `getUser`, and the route keeps its own name.

### What it answers with

A route has several success statuses where a tool has one result, so a route-derived tool answers with the envelope:

```json
{ "status": 200, "body": { "id": "1", "name": "Ada" } }
```

A `404` arrives the same way, with `status: 404`, which is what lets a model tell "no such user" apart from a failure it should retry. A route answering both `200` and `201` needs no union: the envelope carries either.

### Who may call it

The route's own `security` and `accessGate` decide, so authorization stays in [`k.auth`](/docs/authentication) and governs both surfaces from one declaration. A caller who does not carry the identity a route requires cannot reach it by going through a tool.
...
A route's summary was written for a developer reading your docs. A model reads the description to decide whether to call the tool at all, so override it when the two want different words:
...
Every tool in the tree is published, both the ones you declared and the routes you named with `k.tools.fromRoute`. There is nothing to configure: a route reaches a model by being written in the tree, so not publishing one is not writing the line.
