...
An [identity](/docs/authentication) says what a verified caller looks like. This one carries the workspace they belong to:
...
Say in the [auth map](/docs/authentication) which tools require it, beside the routes:
...
`auth.member` is the identity's `context`, typed. The workspace comes from there rather than from `input`, because the model chooses `input`.

Whoever calls the tool supplies the identity, and a tool that declares one will not run without it. In a script, bind it yourself:
