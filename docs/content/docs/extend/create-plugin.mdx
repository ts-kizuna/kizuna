...
A plugin is declared on `k.contract`, so whatever its declaration module imports lands in the contract's import graph, browser bundles included. Keep `node:fs`, a database driver, or a heavy SDK in the server half.
...
A plugin whose props name routes takes them from the `plugins` function instead, and `k.contract` types them:
