...
export const requireScheduler = server.guard('scheduler', ({ bearer, unauthenticated }) =>
    bearer?.token === process.env.CRON_SECRET ? {} : unauthenticated()
);
```
...
<Callout type="warn">
    ...
    guards run — so by then it is parsed on Express and the stream is consumed on Hono and Next. Verify it ahead of kizuna instead:
    framework middleware on the run endpoint, or `express.json({verify})` to keep the raw buffer for a guard to read.
</Callout>
...
Nothing calls `start` on its own. `startJobWorker` does, wherever you want the work done — the same process as the API, or its own:
