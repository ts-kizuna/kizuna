...
import { createJobTransport } from '@ts-kizuna/contract';
...
```ts title="qstash.ts"
import { createJobTransport, type JobMessage } from '@ts-kizuna/contract';
...
```ts title="worker.ts"
import { startJobWorker } from '@ts-kizuna/server/jobs';
