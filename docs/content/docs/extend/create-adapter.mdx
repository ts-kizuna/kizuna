...
description: Build and ship your own ts-kizuna adapter for any HTTP runtime using createAdapter from @ts-kizuna/server.
---

`@ts-kizuna/server` exposes `createAdapter` so third parties can build and ship their own adapters independently. ts-kizuna ships Express, Fastify, Hono, and Next.js.
...
```ts title="adapter.ts"
import { createAdapter, renderJsonResult, parseFetchBody, headersToObject, type AdapterRequest } from '@ts-kizuna/server';
```
