...
Translates any `AdapterResult` (except `raw-response`) to `{ status, headers, body }` using ts-kizuna's defaults — 405 with `Allow` header, RFC 9457 Problem Details, etc. Pass `{ problemDetailsEnabled: false }` to use the legacy `{ message, issues }` shape.
