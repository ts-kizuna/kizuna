...
Translates any `AdapterResult` (except `raw-response`) to `{ status, headers, body }` using ts-kizuna's defaults — 405 with `Allow` header, validation errors as Problem Details (`detail` plus an `errors` array), etc.
