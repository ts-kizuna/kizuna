...
The input is always the canonical problem; only the outgoing bytes change. This is a migration seam, not a way to define custom errors in the contract, and the contract stays Problem Details. Most migrations don't need it at all (use extension members). See [Migrating an existing API](/docs/migration/existing-api).
