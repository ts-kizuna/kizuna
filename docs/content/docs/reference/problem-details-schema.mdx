...
The standard [RFC 9457 Problem Details](https://www.rfc-editor.org/rfc/rfc9457) error response shape used by kizuna. Matches the format produced by [guards](/docs/reference/server-guard) and all built-in error responses (401, 403, 404, 405, 415, etc.).
...
| Source                 | Example                      |
| ---------------------- | ---------------------------- |
| Guards                 | 401 from `unauthenticated()` |
| Access gates           | 403 from the auth map        |
