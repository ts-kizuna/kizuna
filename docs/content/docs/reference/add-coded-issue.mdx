...
To make the client suggest your custom codes, declare them on `kizuna`'s `validation.issueCodes` for that API and pass the contract to [`createClient`](/docs/reference/create-client):
...
Each API has its own `kizuna` factory, so codes stay scoped per API rather than leaking across them.
