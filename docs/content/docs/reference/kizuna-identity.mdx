...
See the [`auth` map reference](/docs/reference/k-contract#the-auth-map) for every value form. From there, each identity needs exactly one [guard](/docs/reference/server-guard), registered on [`new KizunaApi()`](/docs/reference/kizuna-api).
