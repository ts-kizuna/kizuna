...
Pass identities to [`Kizuna.init`](/docs/reference/kizuna-init) under `identities`. Their keys become the names everything else uses — auth map values, guard registrations, and handler context keys:
...
See the [`auth` map reference](/docs/reference/kizuna-init#the-auth-map) for every value form. From there, each identity needs exactly one [guard](/docs/reference/create-server#serverguard), registered on [`server.api`](/docs/reference/create-server#serverapi).
