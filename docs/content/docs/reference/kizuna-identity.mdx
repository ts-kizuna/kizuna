...
An identity describes one kind of authenticated caller: an OpenAPI security scheme (how the credential travels), an optional `context` schema (what a passing [guard](/docs/reference/create-server#serverguard) provides to handlers), and an optional `access` schema (the fields the [`auth` map](/docs/reference/k-contract#the-auth-map) may constrain per route). Omit `context` for an authentication-only identity — a pure gate with no data to hand the handler. For the full walkthrough, see the [Auth guide](/docs/auth).
...
See the [`auth` map reference](/docs/reference/k-contract#the-auth-map) for every value form. From there, each identity needs exactly one [guard](/docs/reference/create-server#serverguard), registered on [`server.api`](/docs/reference/create-server#serverapi).
