...
Plugins go on `new Kizuna()`, beside the rest of your API's vocabulary:
...
The key you choose is the plugin's name everywhere else: handlers read it under `plugins.mcp`, and `new KizunaApi()` takes its config under the same key.
...
Anything live, a store or a transport, goes on `new KizunaApi()`:
...
The split is not arbitrary: `new Kizuna()` reaches your client through the contract, so only pure data can go there. A database handle passed at install would end up in your browser bundle.
...
If a plugin claims a path your contract already uses, `new KizunaApi()` throws and names both.
