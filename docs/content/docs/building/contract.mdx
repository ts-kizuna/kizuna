...
1. **`new Kizuna()`** creates your surface once and exports `k`.
...
You hand the result to [`new KizunaServer()`](/docs/building/router) on the server and [`new KizunaClient()`](/docs/clients/fetch) on every client. This page covers defining it. The [Router](/docs/building/router) and [Mounting](/docs/building/mounting) pages cover implementing and serving it.
...
```ts title="src/contract/contract.ts"
import { k } from './k';
...
Scheduled work is declared on the contract too. [`k.jobs`](/docs/reference/k-jobs) takes the identity every job requires and the jobs themselves, and the result goes under `jobs` on `k.contract`, alongside `routes` rather than inside it:
...
Jobs carry their own identity, so they never appear in the `auth` map, and they stay out of `contract.routes`, the OpenAPI document, and the generated Swift, Kotlin, and MCP surfaces. The [Jobs guide](/docs/jobs) covers declaring, handling, and triggering them.
