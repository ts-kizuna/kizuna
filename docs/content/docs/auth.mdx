...
When a gated field is expensive to resolve, mark it `.optional()` and resolve it only where a route gates it — the guard reads `gatedFields` to know when. See [`createGuard`](/docs/reference/create-guard#resolving-a-field-only-when-its-gated).
...
The [`createClient`](/docs/reference/create-client) client sends the credential in `baseHeaders`; the server's guards enforce the contract's auth and return the typed `401`/`403`:
