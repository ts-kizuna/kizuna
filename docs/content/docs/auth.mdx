...
Split into its own file, a handler keeps the typed `auth` via [`RouteHandler`](/docs/reference/create-server#serverrouter)`<typeof contract.routes.members.listMembers>`.
