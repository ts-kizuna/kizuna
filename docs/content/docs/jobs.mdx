...
`k.jobs` declares background work on the contract. A job has no path and no method.

## Declaring jobs

The first argument is the [identity](/docs/reference/kizuna-identity) every job requires.
...
Pass the result to [`k.contract`](/docs/reference/k-contract) under `jobs`:
...
[`server.jobs`](/docs/reference/server-jobs) mirrors the declaration, nesting included:
...
Pass the handlers to [`server.api`](/docs/reference/server-api) next to the router:
...
Configure both on [`new Kizuna()`](/docs/reference/kizuna) under `settings.jobs`:
