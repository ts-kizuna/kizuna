...
More than one instance is fine now, as long as the queue drops repeated keys. You can also drop `startJobs` and hand the schedules to the queue's own scheduler with [`register`](/docs/extend/create-job-transport#register).
...
`POST /jobs/run` runs the one job the body names, validating `input` against that job's schema and answering `422` when it does not fit. The status it answers with is the job's own, so a queue reads the [retry contract](#the-status-you-return-is-the-retry-contract) straight off it.
