...
Nothing else to mount. The endpoint answers at `/mcp` by default. On Next.js it is served by the catch-all route file that already serves your contract, so with `basePath: '/api'` it lands at `/api/mcp`.
...
- **Tool description** — from the route's `@summary`/`@description` tags, or its `summary` and `description`, falling back to `METHOD /path`
- **Tool input** — structured as `{ params, query, body }` matching the route's schemas, with each field described by its `@description`
...
An assistant works from what your tools say about themselves, so the tags you write in the contract are what make the tools usable: `@description` on routes and fields, plus `@example` values it can copy. It needs the one-time build step in [JSDoc](/docs/jsdoc); without it the tools fall back to `summary`/`description` and `.meta({ description })`.
