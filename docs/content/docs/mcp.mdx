...
MCP is a [plugin](/docs/plugins), declared on `new Kizuna()` and served by `api.mount`:
...
Public routes (`false` in the access map) run without guards, credentials or not.
