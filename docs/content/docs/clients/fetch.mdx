...
A [streaming route](/docs/streaming) returns `stream`, a typed async sequence, where other routes return `body`. Error statuses still return a buffered `body`, so the same `status` narrowing picks the right shape:

```ts
const result = await apiClient.activity.streamActivity({
    params: {
        id: '1',
    },
});
...
if (result.status === 200) {
    for await (const event of result.stream) {
        // event is typed from the route's `event` schema
    }
} else {
    // result.status === 404, result.body is a Problem Details object
}
```

Breaking out of the `for await` loop cancels the request.
