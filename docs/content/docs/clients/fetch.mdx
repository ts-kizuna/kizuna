...
Bodies round-trip [native types](/docs/reference/native-types). The client serializes `Date`, `bigint`, and `URL` values in the request body, and revives the same fields in the response, so a `z.date()` response field really is a `Date` at runtime, exactly as its type says. A `BinarySchema` response arrives as a `Uint8Array`.

Revival is driven by the contract's schemas and never validates: fields the contract does not declare pass through untouched, so a newer server's extra fields do not break an older client. Undeclared statuses arrive as plain parsed JSON.
