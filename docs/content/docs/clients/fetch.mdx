...
```ts
// routes declares: headers: z.object({ 'x-request-id': z.string() })
const { body } = await apiClient.users.getUser({
...
If your routes does not declare a `400`, you don't need this — `result.status === 400` narrows the body to `ValidationError` directly.
