...
The CLI reads the contract at runtime. Add the generated file to your Kotlin project as a regular source file. Pass `--package` so the file declares a package matching its directory (Android/JVM projects expect this).
...
Each method returns a `Result` holding the decoded `body`, and **throws** a typed `Failure` for any error status. The happy path reads `response.body`:

```kotlin
println(page.body.users)
```

Void routes return `Unit`, so call them and let errors propagate:
