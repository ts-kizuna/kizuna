...
By default, field names are emitted verbatim, so `total_count` on the wire stays `total_count` in the generated type. The types mirror the wire shape, so it's easy to see what came from the API. The generated file carries a `@file:Suppress("PropertyName", "ConstructorParameterNaming")` header so the Kotlin naming inspection doesn't flag those underscores.

Pass `--camel-case` to convert wire fields to camelCase properties (`total_count` becomes `totalCount`), preserving the wire name via `@SerialName`. The suppression header is then omitted.
