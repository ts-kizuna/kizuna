...
Commit the generated file, and have CI regenerate it and diff, which catches the change that landed without it:
...
That proves the client matches the contract. Compiling it proves the app still does:
...
### Tip: regenerate on save

Watch the contract while you work and the client is current by the time you commit, so the check never has anything to catch:

```json title="package.json"
{
    "scripts": {
        "dev": "tsdown --watch --onSuccess 'pnpm generate:kotlin'"
    }
}
```
