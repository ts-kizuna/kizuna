...
export interface Contract<
    ...
    ToolAuth extends Record<string, unknown> = Record<string, unknown>,
> {
    /**
    ...
    tools?: Tools_;
    /**
     * The tool `auth` map passed to `k.contract`, nested the way the tool tree
     * is. Carried so the server surface can type each handler's `auth`.
     */
    toolAuth?: ToolAuth;
    /**
...
export function assembleContract<
    ...
    const ToolAuth extends Record<string, unknown> = Record<string, never>,
>(config: {
    routes: R;
    jobs?: Jobs_;
    jobsConfig?: JobsConfig;
    ...
    plugins?: Plugins;
}): Contract<R, Tags, Codes, Schemes, Auth, RequestContext, Plugins, Jobs_, Tools_, ToolAuth> {
    return {
...
/**
 * The tool auth map a contract was assembled with.
 */
export type ToolAuthOf<C extends Contract> = Exclude<C['toolAuth'], undefined>;
