...
const holder = Kizuna.identity.bearer({
    context: z.object({
        userId: z.string(),
    }),
});
    ...
    k.contract({
        ...
        access: {
            items: {
                ...
                both: {
                    auth: 'holder',
                    permission: ['viewInvoices', 'exportLedger'],
                },
                either: {
                    auth: 'holder',
                    permission: {
...
type PermissionMethods = Record<string, (record?: unknown) => Promise<boolean>>;
...
describe('laziness', () => {
    it('resolves a permission once per request even when asked twice', async () => {
        ...
        await run('/open', { ...allow, viewInvoices: implementation }, async ({ permission }: { permission: PermissionMethods }) => {
...
describe('can in a handler', () => {
    it('answers about the record it is given', async () => {
        ...
        const result = await run('/open', allow, async ({ permission }: { permission: PermissionMethods }) => {
            seen.push(await permission.promoteMember!({ id: 'usr_9' }));
            seen.push(await permission.promoteMember!({ id: 'usr_44' }));
            return okHandler();
        });
    ...
    it('awaits an async predicate', async () => {
        ...
        await run(
            ...
            async ({ permission }: { permission: PermissionMethods }) => {
                seen.push(await permission.promoteMember!({ id: 'usr_9' }));
                return okHandler();
            }
        );
...
describe('misconfiguration', () => {
    ...
    it('is a handler error when a record-scoped permission is asked without its record', async () => {
        const result = await run('/open', allow, async ({ permission }: { permission: PermissionMethods }) => {
            await permission.promoteMember!();
            return okHandler();
        });
        expect(result.kind).toBe('handler-error');
