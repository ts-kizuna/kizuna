...
/**
 * Map values are the text after `@deprecated` (empty string for a bare tag).
 * Swift uses the text for `@available(*, deprecated, message: "...")`.
 */
export type DeprecationMap = {
    routes: Map<string, string>;
    fields: Map<string, Map<string, string>>;
    /**
     * Deprecated fields keyed by schema `meta.id`, independent of route paths.
     * Used to annotate named structs that are only reachable via generic wrappers
     * (e.g. `Pagination<T>`) where the route-level field path can't be computed.
     */
...
type IdentifierResolver = (node: ts.Identifier) => ts.Expression | undefined;

const resolveImportPath = (dir: string, specifier: string): string | undefined => {
...
const buildFileScope = (filePath: string, cache: Map<string, Scope>): Scope => {
...
const makeResolverWithCache = (contractPath: string): { resolve: IdentifierResolver; cache: Map<string, Scope> } => {
...
const collectFieldDeprecations = (schemaNode: ts.Node, prefix: string, into: Map<string, string>, resolve: IdentifierResolver): void => {
    ...
    // paths include the wrapper's own field structure (e.g. responses.200.items.field).
    if (ts.isCallExpression(resolved) && ts.isIdentifier(resolved.expression)) {
        ...
        if (funcExpr && (ts.isArrowFunction(funcExpr) || ts.isFunctionExpression(funcExpr))) {
            collectFieldDeprecations(getFunctionBody(funcExpr), prefix, into, makeScopedResolver(funcExpr, resolved.arguments, resolve));
            return;
        }
    }

    const extendBase = findExtendBase(resolved);
    if (extendBase) collectFieldDeprecations(extendBase, prefix, into, resolve);
    ...
    for (const property of objectLiteral.properties) {
        ...
        const message = readDeprecatedMessage(property);
        if (message !== undefined) into.set(fieldPath, message);
        collectFieldDeprecations(property.initializer, fieldPath, into, resolve);
    }
};
