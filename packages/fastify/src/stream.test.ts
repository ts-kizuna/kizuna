...
import { fastifyKizuna, createServer } from './server.js';
...
// `reply.hijack()` takes the socket over, which `app.inject` does not model, so these go over a real port.
let baseUrl: string;
...
describe('Fastify streaming', () => {
    ...
    it('streams the writer form', async () => {
        const response = await fetch(`${baseUrl}/stream/writer`);
    ...
    it('does not open a stream when the handler throws before the first event', async () => {
        const response = await fetch(`${baseUrl}/stream/throws-early`);
