...
describe('Hono streaming', () => {
    ...
    it('streams the writer form', async () => {
        const response = await app.request('/stream/writer');
    ...
    it('does not open a stream when the handler throws before the first event', async () => {
        const response = await app.request('/stream/throws-early');
