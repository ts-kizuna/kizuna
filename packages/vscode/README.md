...
Shows [ts-kizuna](https://ts-kizuna.com) deprecations where you code: deprecated routes and fields are struck through everywhere the contract's types flow, hovering shows the migration message, and completions strike through deprecated entries.
...
The extension carries `@ts-kizuna/typescript-plugin`, a TypeScript language service plugin. Editors without the extension load it through `tsconfig.json`, covered in the [documentation](https://ts-kizuna.com/docs/vscode#other-editors).
