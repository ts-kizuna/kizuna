...
// tsserver plugins are CommonJS, so the test loads the built dist the same way
// tsserver does. Run `pnpm build` in this package after changing index.cts.
...
const fixturePath = path.resolve(fixtureDir, 'poc.fixture.ts');
...
/**
 * The position of `property` inside the first occurrence of `snippet` in the
 * fixture.
 */
