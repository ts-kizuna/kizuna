 ...
 * past it.
 */
const buildPermissionMethods = (
    routeKey: string,
    params: Record<string, string>,
    securityContext: Record<string, unknown>,
    permissions?: PermissionMap,
    handlerContext?: Record<string, unknown>
): { permission: PermissionMethods; resolve: (name: string) => Promise<PermissionOutcome> } => {
    ...
    const permission: PermissionMethods = {};
    for (const name of Object.keys(permissions ?? {})) {
        permission[name] = async (record?: unknown) => {
...
const executeToolCall = async (
    ...
    const { permission, resolve } = buildPermissionMethods(routeKey, params, guardOutcome.securityContext, permissions, handlerContext);
