 ...
 */
const contractTools = k.tools(({ toolFromRoutes }) => ({
    users: {
        listUsers: toolFromRoutes(contractRoutes.users.listUsers),
        getUser: toolFromRoutes(contractRoutes.users.getUser),
        createUser: toolFromRoutes(contractRoutes.users.createUser),
    },
    health: toolFromRoutes(contractRoutes.health),
    pingUser: toolFromRoutes(contractRoutes.pingUser),
    deleteUser: toolFromRoutes(contractRoutes.deleteUser),
    updateUser: toolFromRoutes(contractRoutes.updateUser),
}));
...
describe('MCP server: guards', () => {
    ...
    const securedTools = securedK.tools(({ toolFromRoutes }) => ({
        api: {
            publicRoute: toolFromRoutes(securedRoutes.publicRoute),
            whoAmI: toolFromRoutes(securedRoutes.whoAmI),
            ownerOnly: toolFromRoutes(securedRoutes.ownerOnly),
        },
    }));
...
describe('MCP server: request context in guards', () => {
    ...
    const contextRoutes = contextK.routes({
        whoAmI: {
            method: 'GET',
            path: '/who-am-i',
            responses: {
    ...
    const contextTools = contextK.tools(({ toolFromRoutes }) => ({
        api: {
            whoAmI: toolFromRoutes(contextRoutes.whoAmI),
        },
        reindex: {
    ...
    const contextContract = contextK.contract({
        ...
        tools: contextTools,
        toolAccessControl: contextK.accessControl.tools(contextTools, {
            reindex: 'user',
        }),
        accessControl: {
            api: 'user',
